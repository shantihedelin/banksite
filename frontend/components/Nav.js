import Link from "next/link";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faBars } from "@fortawesome/free-solid-svg-icons";

//TODO: make the hamburger icon work on mobile devices
//TODO: more margin between the Links on desktop view
//TODO: make a Link to account on mobile devices when logged in, instead
//TODO: of still showing the Sign in button

const logo =
  "data:image/png;base64,";

export default function Nav() {
  return (
    <>
      <nav className="flex justify-between mb-6 md:hidden">
        <div>
          <Link
            className="text-2xl no-underline pl-3 flex justify-center items-center space-x-2"
            href={"/"}
          >
            <img className="w-12 h-12" src={logo} />
            <h1 className="text-xl text-black">Nova Bank </h1>
          </Link>
        </div>

        <div className="flex flex-row space-x-6 justify-center items-center pr-3">
          <Link
            className="text-lg no-underline border-solid rounded-3xl px-3 py-1 signIn-btn"
            href={"/login"}
          >
            Sign in
          </Link>
          <button className="bg-transparent border-none cursor-pointer">
            <FontAwesomeIcon icon={faBars} className="icon" />
          </button>
        </div>
      </nav>

      <nav className="hidden md:flex space-x-4 justify-between items-center">
        <div>
          <Link
            className="text-2xl no-underline pl-3 flex justify-center items-center space-x-2"
            href={"/"}
          >
            <img
              className="w-12 h-12"
              src="data:image/png;base64,"
            />
            <h1 className="text-xl text-black">Nova Bank </h1>
          </Link>
        </div>
        <div className="flex flex-row space-x-4 pr-4">
          <Link
            className="text-lg no-underline border-solid rounded-3xl px-3 py-1 signIn-btn"
            href={"/login"}
          >
            Sign in
          </Link>
          <Link
            className="text-lg text-gray-800 no-underline py-1 hover:text-slate-400 nav-btn"
            href={"/register"}
          >
            Register
          </Link>
          <Link
            className="text-lg text-gray-800 no-underline py-1 hover:text-slate-400 nav-btn"
            href={"/account"}
          >
            Account
          </Link>
        </div>
      </nav>
    </>
  );
}
